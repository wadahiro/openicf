...
public class TestConfigurationReaderTest {

    @Test
    public void testLoadGroovyConfigFileIssue393() {
        Map<?, ?> props = TestConfigurationReader.loadGroovyConfigFile(TestConfigurationReaderTest.class.getResource("properties.groovy"));
        assertEquals(Integer.class, props.get("prop.integerclass"));
    }
    
    @Test
    public void loadProjectPropertiesAsResource(){
    	String oldTestConfig = System.getProperty("testConfig");
    	System.setProperty("testConfig", "myconfig");
    	try{
	    	Map<?,?> properties = TestConfigurationReader.loadConnectorConfigurationAsResource(DummyConnector.class, new ConfigClassLoader());
