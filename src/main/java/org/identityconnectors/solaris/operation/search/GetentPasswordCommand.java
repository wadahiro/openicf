 ...
 *
 */
public class GetentPasswordCommand {
	
	private static final Log log = Log.getLog(GetentPasswordCommand.class);
	...
	public static SolarisEntry getEntry(String line, String username) {
        ...
        if (!username.equals(foundUser)) {
            log.warn("The fetched username differs from what was expected (in getent passwd): fetched = '" +  foundUser + "', expected = '" + username + "'.");
            return null;
        }
